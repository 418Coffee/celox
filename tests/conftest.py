...
def _http_message_chunked() -> "tuple[bytes, list[str]]":
    ...
    chunk_sizes = [math.floor(i) for i in np.random.uniform(size=random.randint(2, 5),low=1, high=2000)]
...
def _http_message_chunked_trailers() -> "tuple[bytes, list[str]]":
    ...
    yield b"\r\n0\r\n"
    yield \
        b"Expires: Thu, 17 Mar 2022 12:47:32 GMT\r\n" \
...
_http_message_chunked_trailers_bytes = [chunk for chunk in _http_message_chunked_trailers()]
...
async def direct_connection(nursery, request):
    listeners = await nursery.start(
        partial(trio.serve_tcp, request.param, port=0)
    )
    ...
    if sockaddr[0] == "::":
        host = "::1"
    port = sockaddr[1]
    async with DirectConnection(host, port, create_ssl_context(), Timeout(5, 5, 5, 5)) as conn:
        yield conn

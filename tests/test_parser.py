...
def test_parse_complete_http_message(http_message: bytes):
    ...
    # \r\n\r\n is stripped
    assert len(status_and_headers) == len(http_message) - 4
...
def test_parse_complete_http_message_with_body(http_message: bytes):
    http_message += b"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
    buf = ReceiveBuffer(http_message)
    status_and_headers = buf.read()
    body = buf._buf
    # \r\n\r\n is stripped
    assert len(status_and_headers) == len(http_message) - 4 - len(body)
